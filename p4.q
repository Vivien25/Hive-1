...
SELECT getrank.team FROM
(SELECT error_data.team, error_data.errors, DENSE_RANK() OVER (ORDER BY error_data.errors DESC) as ranked 
FROM (SELECT team, SUM(errors) AS errors FROM fielding WHERE year = 2001 GROUP BY team) error_data)getrank
