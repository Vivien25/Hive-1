...
SELECT getrank.weight FROM
(SELECT weight_count.weight, weight_count.count, DENSE_RANK() OVER (ORDER BY weight_count.count DESC) as ranked 
FROM (SELECT weight, count(id) AS count FROM master GROUP BY weight) weight_count) getrank
WHERE getrank.ranked =2;