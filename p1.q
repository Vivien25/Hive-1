...
SELECT m.bcity FROM master m JOIN
(SELECT rank.id FROM (SELECT AB_data.id, AB_data.at_bats, DENSE_RANK() OVER (ORDER BY AB_data.at_bats DESC) as ranked 
FROM (SELECT id, SUM(ab) as at_bats FROM batting GROUP BY id)AB_data) rank
WHERE rank.ranked <2) top1
WHERE m.id = top1.id;
